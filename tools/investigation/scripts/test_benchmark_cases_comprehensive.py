#!/usr/bin/env python3
"""Comprehensive test of VMEC++ with multiple benchmark cases Tests both symmetric and
...
def test_single_case(filepath, name, filetype):
    ...
    try:
        ...
        if filetype == "json" or filepath.endswith(".json"):
            ...
            print("  WARNING  INDATA files need special handling, skipping...")
        ...
        print(
            f"  Parameters: lasym={vmec_input.lasym}, mpol={vmec_input.mpol}, ntor={vmec_input.ntor}"
        )
...
def main():
    ...
    # Count by symmetry
    symmetric_success = sum(
        1 for r in results if r["status"] == "success" and r["lasym"] == False
    )
    asymmetric_success = sum(
        1 for r in results if r["status"] == "success" and r["lasym"] == True
    )

    print(f"\nTotal cases tested: {total}")
    print(
        f"[done] Successful: {success} ({symmetric_success} symmetric, {asymmetric_success} asymmetric)"
    )
    print(f"WARNING  Skipped: {skipped}")
    print(f"[fail] Failed: {total - success - skipped}")
    ...
    if aznorm_errors == 0 and asymmetric_success > 0:
        print("[success] SUCCESS: azNorm=0 error has been eliminated!")
        print(f"   {asymmetric_success} asymmetric cases ran successfully")
    elif aznorm_errors > 0:
        print("[fail] FAILURE: azNorm=0 error still occurring")

    if size_errors > 0:
        print(f"\nWARNING  Note: {size_errors} cases have array size validation issues")
