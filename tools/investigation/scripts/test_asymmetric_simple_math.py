...
for i, t in enumerate(theta):
    if i % 3 == 0:  # Every 90 degrees
        print(f"  theta={t:.3f} ({t*180/np.pi:.1f}Â degrees): R={R_expected[i]:.6f}, Z={Z_expected[i]:.6f}")

print("\nDetailed check at theta=0:")
...
print("\nCheck at theta=pi/2:")
print(f"  R = RBC(0,0)*cos(0) + RBC(1,0)*cos(pi/2) + RBS(1,0)*sin(pi/2)")
...
for t, r, z in vmecpp_output:
    ...
    if i < len(R_expected):
        ...
        print(f"  theta={t:.3f}: R={r:.6f} (error={r_err:+.6f}), Z={z:.6f} (error={z_err:+.6f})")

# Analysis
print("\n=== ANALYSIS ===")
print("The VMEC++ values suggest:")
print("1. At theta=0: R=1.424264 instead of 1.3")
