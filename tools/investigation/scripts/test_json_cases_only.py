...
def test_json_case(filepath):
    ...
    try:
        ...
        print(
            f"  lasym={vmec_input.lasym}, mpol={vmec_input.mpol}, ntor={vmec_input.ntor}"
        )
        ...
        print(f"  [done] SUCCESS! Volume={output.volume_p:.3f}")
        ...
        print(f"  [fail] FAILED: {error_msg[:80]}...")
...
def main():
    ...
    print(
        f"[done] Success: {success_count} (including {asymmetric_success} asymmetric)"
    )
    print(f"[fail] azNorm errors: {aznorm_count}")
    print(f"[fail] Other errors: {other_count}")

    if aznorm_count == 0 and asymmetric_success > 0:
        print("\n[success] azNorm=0 error eliminated for asymmetric cases!")
