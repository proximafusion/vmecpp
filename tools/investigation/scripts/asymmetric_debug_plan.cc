...
if (s_.lasym) {
  ...
  for (int i = 0; i < std::min(5, (int)physical_x.rmnsc.size()); ++i) {
    std::cout << "  rmnsc[" << i << "]=" << physical_x.rmnsc[i] << " zmncc["
              << i << "]=" << physical_x.zmncc[i] << "\n";
  }
}
#endif
...
if (jF == 1) {  // Only debug first interior surface
  ...
  for (int kl = 0; kl < std::min(4, s_.nZnT); ++kl) {
    ...
    local_arNorm +=
        ruFull[idx_kl] * ruFull[idx_kl] * s_.wInt[kl % s_.nThetaEff];
    local_azNorm +=
        zuFull[idx_kl] * zuFull[idx_kl] * s_.wInt[kl % s_.nThetaEff];
  }
