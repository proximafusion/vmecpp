...
print(
    f"Loaded: lasym={vmec_input.lasym}, mpol={vmec_input.mpol}, ntor={vmec_input.ntor}"
)
...
try:
    ...
    print("[done] SUCCESS! No azNorm=0 error!")
    print(f"   Volume={output.volume_p:.3f}")
    print(f"   Beta={output.beta:.6f}")
    print("   The azNorm=0 fix is working!")
except Exception as e:
    error_msg = str(e)
    if "azNorm should never be 0.0" in error_msg:
        print("[fail] FAILED: azNorm=0 error still present!")
        print("   The asymmetric Fourier transform is not being used correctly")
    else:
