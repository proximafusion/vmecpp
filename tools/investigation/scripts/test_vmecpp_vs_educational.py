...
def run_educational_vmec(input_file, work_dir):
    ...
    result = subprocess.run(
        [edu_vmec, input_name], cwd=work_dir, capture_output=True, text=True, timeout=60
    )
...
for test_case in test_cases:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if edu_result.returncode == 0:
            ...
            if (
                "azNorm should never be 0.0" in edu_result.stdout
                or "azNorm should never be 0.0" in edu_result.stderr
            ):
        ...
        if json_file and json_file.exists():
            ...
            if vmecpp_result and vmecpp_result.returncode == 0:
                ...
                if (
                    "azNorm should never be 0.0" in vmecpp_result.stdout
                    or "azNorm should never be 0.0" in vmecpp_result.stderr
                ):
                    print("   Error: azNorm=0 detected")
