...
def parse_pt_ph_from_namelist(namelist_file):
    ...
    # Parse PT_TYPE
    pt_type_match = re.search(
        r'PT_TYPE\s*=\s*["\']([^"\']+)["\']', indata_content, re.IGNORECASE
    )
    pt_type = pt_type_match.group(1) if pt_type_match else "power_series"

    # Parse PH_TYPE
    ph_type_match = re.search(
        r'PH_TYPE\s*=\s*["\']([^"\']+)["\']', indata_content, re.IGNORECASE
    )
    ...
    # Parse AT array
    at_match = re.search(
        r"AT\s*=\s*((?:[\d.eE+-]+\s*)+)", indata_content, re.IGNORECASE
    )
    if at_match:
    ...
    # Parse AH array
    ah_match = re.search(
        r"AH\s*=\s*((?:[\d.eE+-]+\s*)+)", indata_content, re.IGNORECASE
    )
    if ah_match:
