...
def test_spectral_condensation_asymmetric():
    ...
    with open("test_symmetric_quick.json") as f:
        data = json.load(f)
    ...
    try:
        ...
        vmec = vmecpp.run_vmec(input_file=modified_file, verbose=5, checkpoint="forces")
        print("\nVMEC completed successfully!")
        print(
            "If VMEC runs without 'not spectrally condensed' errors, the fix is working"
        )
