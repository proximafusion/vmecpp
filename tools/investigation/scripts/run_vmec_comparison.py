...
def validate_against_reference():
    ...
    try:
        ...
        if rel_diff < 0.001:
            print("[done] VMEC++ MATCHES REFERENCE - VALIDATION PASSED")
        else:
            print("[fail] VMEC++ DIFFERS FROM REFERENCE - VALIDATION FAILED")
...
def main():
    ...
    if not reference_ok:
        print(
            "WARNING  Reference validation failed - proceeding with comparison anyway"
        )
        print()
    ...
    for input_file in input_files:
        ...
        if vmecpp_result.success:
            ...
            print(f"  VMEC++: [done] MHD Energy = {vmecpp_result.wb:.6e}")
        else:
            error = (
