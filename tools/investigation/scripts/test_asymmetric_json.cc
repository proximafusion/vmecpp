...
TEST(AsymmetricJsonTest, LoadExistingJsonFile) {
  ...
  if (!status.ok()) {
    ...
    if (status.message().find("INITIAL JACOBIAN CHANGED SIGN") !=
        std::string::npos) {
      std::cout << "\nWARNING  JACOBIAN SIGN ERROR - This is the key issue!"
                << std::endl;
      std::cout
          << "The initial guess interpolation is causing negative Jacobian"
          << std::endl;
    }
  } else {
    std::cout << "\n[done] SUCCESS! Asymmetric case ran to completion"
              << std::endl;
