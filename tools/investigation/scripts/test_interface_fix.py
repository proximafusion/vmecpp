...
def test_symmetric_case():
    ...
    try:
        ...
        if hasattr(result, "ier"):
            ...
            if result.ier == 0:
                print(
                    "✅ Symmetric case PASSED - No regression from lambda coefficient changes!"
                )
                return True
...
def test_asymmetric_case():
    ...
    try:
        ...
        if hasattr(result, "ier"):
            ...
            if result.ier == 0:
                print(
                    "✅ Asymmetric case PASSED - Lambda coefficient processing is working!"
                )
                return True
            print(f"⚠️  Asymmetric case still has issues: ier={result.ier}")
            print(
                "   (This may require additional priorities: force symmetrization, m=1 constraints, etc.)"
            )
            return False
...
if __name__ == "__main__":
    ...
    if asymmetric_ok:
        print(
            "✅ PROGRESS: Asymmetric mode is now working with lambda coefficient processing"
        )
    else:
        print("⚠️  PARTIAL PROGRESS: Asymmetric mode improved but still needs work")
        print(
            "   Next priorities: force symmetrization, m=1 constraints, axis optimization"
        )
