...
template <class Real, sctl::Integer UPSAMPLE, sctl::Integer PDIM, sctl::Integer RDIM>
void GeneralVirtualCasing(sctl::Vector<Real>& Bext, sctl::Vector<Real>& Bext_eval,
  ...
  auto DotProd = [](sctl::Vector<Real>& AdotB, const sctl::Vector<Real>& A,
    ...
    for (sctl::Long i = 0; i < N; i++) {
      Real s = 0;
      for (sctl::Integer k = 0; k < COORD_DIM; k++) s += A[k * N + i] * Bv[k * N + i];
      AdotB[i] = s;
    }
  };
  ...
  Bext = 0.5 * B + B0 + B1;
  // BIEST's Laplace3D::FxdU is the negative-gradient (field) convention, so its
  // n.B monopole term carries the opposite sign to the AEGIS singularity
  // subtraction; the matching exterior limit is 0.5 B + BiotSavart - Laplace.
  Bext_eval = 0.5 * B + B1 - B0;
}
...
int main(int argc, char** argv) {
  ...
  std::cerr << "biest_driver: wrote " << Nt << "x" << Np << " B_ext (both conventions)\n";
  return 0;
}
