...
def test_equilibrium_rescale():
    ...
    np.testing.assert_allclose(
        oq_rescaled.wout.volume_p, oq_initial.wout.volume_p * (r_scale**3), rtol=1e-5
    )

    # Pressure should scale as b_scale^2
    np.testing.assert_allclose(
        oq_rescaled.wout.pres, oq_initial.wout.pres * (b_scale**2), rtol=1e-5
    )

    # Magnetic field should scale as b_scale
    np.testing.assert_allclose(
        oq_rescaled.wout.bmnc, oq_initial.wout.bmnc * b_scale, rtol=1e-5
    )

    # Betas should be invariant under this scaling (pressure ~ B^2)
    np.testing.assert_allclose(
        oq_rescaled.wout.betatotal, oq_initial.wout.betatotal, rtol=1e-5
    )
