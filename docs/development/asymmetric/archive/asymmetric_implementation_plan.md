...
3. **Symmetrization operation (symrzl)**:
   ...
   // For theta in [0, pi]: total = symmetric + asymmetric
   ```
...
|-----------|--------|-----------------|
| GuessAxis algorithm | [fail] Missing | `boundaries.cc` |
| BadJacobian retry | [fail] Missing | `vmec.cc` |
| Asymmetric totzspa | [wip] Stub only | `ideal_mhd_model.cc` |
| Asymmetric tomnspa | [wip] Stub only | `ideal_mhd_model.cc` |
| Symmetrization (symrzl) | [fail] Missing | `ideal_mhd_model.cc` |
| Asymmetric interpolation | [fail] Wrong | `ideal_mhd_model.cc` |
