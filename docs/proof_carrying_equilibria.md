...
Stellarocq proves identities the reconstruction satisfies rather than assumes. `divergence_free` states that the divergence of the reconstructed field is exactly zero, and `pressure_is_a_flux_function` that dp/ds reads neither angle. The cells that bound the residual also enclose flux-surface integrals, so `dV/ds`, the enclosed currents and the terms of the Mercier criterion come out as intervals, and `mercier_geodesic_nonpositive` proves the sign of the geodesic term by Cauchy-Schwarz where no enclosure decides it. `harm_encloses` encloses the discrete Fourier harmonics of the residual over the angles of a node. Departure from quasisymmetry is bounded from the field, without Boozer coordinates, and decided from those harmonics: the two-term ratio is a flux function only if every harmonic of its defect is one constant times the matching harmonic of its product, and two disjoint certified ratios refute that. The harmonics of `|B|` in the Boozer angles are certified through the stream function the certificate carries. A residual component proven bounded away from zero over a cell shows that no field of the certified form is in force balance there. `newton_correct` gives a unique zero of any system of expressions in a box from an interval Newton test, and `colloc_correct` applies it to the residual collocated over a band of surfaces with their Fourier coefficients as the unknowns, a discrete equilibrium of the reconstruction. The physical assumptions a statement needs are propositions in `theories/Hypotheses.v`, each with what would falsify it.

## Solver oracle

The checker shares no code with VMEC++ or Fortran VMEC, so a certificate that fails where one has to hold points at the solver. A stellarator-symmetric equilibrium certified through the non-stellarator-symmetric reconstruction, with its antisymmetric coefficients set to zero, has to hold at the same bounds, which is the reduction `tests/test_lasym.py` requires of the solver. Checking that reduction found [#788](https://github.com/proximafusion/vmecpp/issues/788), fixed in [#789](https://github.com/proximafusion/vmecpp/pull/789), and the same defect in educational_VMEC ([#27](https://github.com/jonathanschilling/educational_VMEC/issues/27) there). `gen/oracle.py` runs four such invariances against VMEC++, each named by a theorem: an axisymmetric input through the three-dimensional path, a symmetric input through the asymmetric path, a restart with the `(0,0)` lambda coefficient set, and a five-period stellarator over one period and over the torus. All four hold to rounding.
...
Circular tokamak of the test data, `iota = 0.9 - 0.64 s`, `iota = 1/2` at `s = 0.625`, `(1,1)` boundary ripple 0.03: the `(2,1)` harmonic of `r_s` on that surface is 2.1e-6, 2.2e-6 and 1.8e-6 at 65, 129 and 257 surfaces and linear in the ripple; every other harmonic falls fourfold per doubling. With `iota = 0.62 - 0.1 s` the same harmonic falls fourfold. A `(2,1)` ripple imposed directly is regular through beta 4.7 per cent (`gen/resonance.py`). The pointwise residual of the rippled runs, near 3e-4 on the fine grids, is 99.9 per cent at `n = 5` and `6`, past `ntor = 4`.

`wout_up_down_asym`, `s = 0.125`, sixteen surfaces: the certified Mercier current term is -3.7e-3, the file's -6.1e-4. At 33 to 257 surfaces both fall to zero and the file's sign of `DMerc` survives.
...
One, two and three surfaces of `wout_solovev` have exactly one zero of the collocated residual within 3e-11, 1.3e-9 and 1.6e-9 of the wout's coefficients; five do not pass the test. The rational surface of the rippled tokamak and the same surface of its control both pass at 33 surfaces, with contraction constants 0.29 and 0.25, and neither at 65, where the Jacobian's condition number is five times larger (`gen/newton_colloc.py`).
