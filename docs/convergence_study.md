...
Branch: `improve-convergence` (includes PR #616 merged). All behavior changes
are opt-in via environment variables; with none set, the solver is unchanged
(verified: identical iteration counts and physics on cth/w7x, and the full
Python test suite passes: 191 passed, 2 skipped). Pre-commit (clang-format
etc.) is clean on all touched files.

The rollback-backup-after-interpolation fix is **default behavior**, split
out as its own commit (`fix-multigrid-backup-ordering`, stacked under this
branch): it matches PARVMEC/VMEC2000 (>= 2017-01-24) rather than VMEC 8.52
on this point. The remaining levers are env-gated:

```
VMECPP_MULTIGRID_INTERP=cubic  # 4-point Lagrange radial interpolation
VMECPP_DELT_START=0.5          # continuation stages enter at 0.5 * delt
VMECPP_DELT_RECOVERY=1         # delt grows back on sustained progress
VMECPP_PRESERVE_INTERP_SEED=1  # axis-recovery keeps the interpolated seed
```

The experimental 2D preconditioner (`VMECPP_PREC2D`, Finding 5) was removed
from this branch after its evaluation came out negative; see the branch
history or PR #616 for the code.

Proposed defaults after review: cubic interpolation + delt start/recovery +
seed preservation change trajectories (not converged physics) and should
probably become the default, with the old behavior kept selectable as
