...
- closes the bootstrap current inside the solve: with `bootstrap_current`, the enclosed toroidal current profile follows the Redl closure evaluated on the iterating field from the kinetic profiles `bootstrap_ne`, `bootstrap_te` and `bootstrap_ti`, and the run converges to an equilibrium whose current is the one its own pressure drives
