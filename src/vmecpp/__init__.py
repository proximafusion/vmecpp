...
class VmecWOut(BaseModelWithNumpy):
    ...
    """Fourier coefficients of the covariant magnetic field component `B_zeta` on the
    half-grid."""
    ...
    """Fourier coefficients of the contravariant magnetic field component `B^zeta` on
    the half-grid."""
...
class Threed1Volumetrics(BaseModelWithNumpy):
    ...
    int_bpol: float
    """Total poloidal magnetic field energy `B_zeta^2/(2 mu0)` integrated over the
    plasma volume."""
...
class JxBOut(BaseModelWithNumpy):
    ...
    `i_theta = (d B_s/dZeta - d B_zeta/ds)/mu0`
    """

    izeta: jt.Float[np.ndarray, "num_half nZnT"]
    """Toroidal surface current.

    `i_zeta = (-d B_s/dTheta + d B_theta/ds)/mu0`
    """
    ...
    `j^s = (d B_theta/dZeta - d B_zeta/dTheta)/(mu0 * V')`
    """
