...
TEST(TestRadialProfiles, CheckSolovevSingleThreaded) {
  ...
  EXPECT_TRUE(IsCloseRelAbs(profil1d["r00"],
                            vmec_indata->rbc(0, vmec_indata->ntor), tolerance));
      ...
      for (int m = 0; m < s.mpol; ++m) {
        if (m % 2 == 0) {  // m is even
          EXPECT_TRUE(IsCloseRelAbs(
              ...
              p.scalxc[(jF - vmec.r_[0]->nsMinF1) * 2 + kEvenParity],
              tolerance));
        } else {  // m is odd
          EXPECT_TRUE(IsCloseRelAbs(
              profil3d["scalxc"][jF][n][m],
              p.scalxc[(jF - vmec.r_[0]->nsMinF1) * 2 + kOddParity],
              tolerance));
        }
      }  // m
    }  // n
