...
#include "fftx_prdftbat_cpu_decls.h"
...
transformTuple_t *fftx_prdftbat_cpu_Tuple(fftx::point_t<4> req) {
  ...
  for (indx = 0; indx < numentries; indx++) {
    if (req[0] == AllSizes4_CPU[indx][0] && req[1] == AllSizes4_CPU[indx][1] &&
      ...
      if (wp != NULL) {
        *wp = fftx_prdftbat_CPU_Tuples[indx];
      }
      break;
    }
  }
...
void fftx_prdftbat_cpu_Run(fftx::point_t<4> req, double *output,
                           double *input) {
  transformTuple_t *wp = fftx_prdftbat_cpu_Tuple(req);
  if (wp == NULL)
...
int fftx_prdftbat_cpu_python_init_wrapper(int *req) {
  ...
  rsz[2] = req[2];
  rsz[3] = req[3];
  transformTuple_t *wp = fftx_prdftbat_cpu_Tuple(rsz);
  if (wp == NULL)
