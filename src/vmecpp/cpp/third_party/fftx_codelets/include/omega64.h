...
/* More accurate cosine.  Renamed to avoid clashing with glibc 2.41+ which
   exports cospi/sinpi as math intrinsics; macros below remap the names so
   SPIRAL-generated code keeps compiling. */
static double spiral_cospi(double a) { /*return cos(PI*a); */
  double sign = +1.0;
  while (a >= 2.0) a -= 2.0;
  while (a < 0.0) a += 2.0;
  if (a >= 1.0) {
...
static double spiral_sinpi(double a) { /*return sin(PI*a); */
  return spiral_cospi(0.5 - a);
}

#define cospi spiral_cospi
#define sinpi spiral_sinpi
