...
absl::Status MagneticField(
  ...
  const Vector3d &direction = *infinite_straight_filament.direction_;
  ...
  const Vector3d &origin = infinite_straight_filament.origin_.value();
...
absl::Status MagneticField(
  ...
  for (const SerialCircuit &serial_circuit :
    ...
    for (const Coil &coil : serial_circuit.coils_) {
      ...
      double current = 0.0;
      if (coil.num_windings_.has_value()) {
        current = serial_circuit.current_.value() * coil.num_windings_.value();
      } else {
        // assume num_windings = 1, if not provided
        current = serial_circuit.current_.value();
      }
...
absl::StatusOr<double> LinkingCurrent(
  ...
  const int num_axis_points = 2 * (2 * maximum_mode_number + 1) *
                              magnetic_configuration.num_field_periods_.value();
