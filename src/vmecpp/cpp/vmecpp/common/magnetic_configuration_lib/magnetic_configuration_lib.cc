  ...
  for (std::string raw_line; std::getline(m_makegrid_coils_ss, raw_line);) {
    ...
    if (num_line_parts == 4 || num_line_parts == 6) {
      ...
      if (num_line_parts == 6) {
        ...
        if (x.size() == 1) {
          // circular filament: use mutable accessor to get/create and then set
          // fields directly
          CircularFilament* circular_filament =
              current_carrier->mutable_circular_filament();
          circular_filament->name = current_carrier_name;
          circular_filament->radius = x.at(0);

          circular_filament->center.set_x(0.0);
          circular_filament->center.set_y(0.0);
          circular_filament->center.set_z(z.at(0));
          ...
          if (!coil->num_windings_) {
            return absl::InternalError(
                "Coil num_windings should have been set but wasn't.");
          }
          double coil_num_windings =
              *coil->num_windings_;  // Dereference optional

          for (size_t i = 1; i < w.size() - 1; ++i) {
            if (w.at(i) != coil_num_windings) {
              std::stringstream error_message;
              error_message << "Number of windings mismatch at point " << i
                            << " (" << w.at(i) << " vs " << coil_num_windings
                            << ") in coil ending with name '"
                            << current_carrier_name << "'";
          ...
          polygon_filament->name = current_carrier_name;

          // Add vertices using the helper method
          polygon_filament->vertices.resize(3, x.size());
          for (size_t i = 0; i < x.size(); ++i) {
            Eigen::Vector3d vertex{x[i], y[i], z[i]};
            polygon_filament->vertices(Eigen::all, i) = vertex;
          }
        }
...
absl::StatusOr<MagneticConfiguration> ImportMagneticConfigurationFromMakegrid(
  ...
  bool in_filament_section =
      false;  // Flag to track if we are parsing filaments
      ...
      if (absl::StartsWith(stripped_line, "periods")) {
        ...
        if (line_parts.size() != 2) {
          return absl::InvalidArgumentError(
              absl::StrCat("Expected 2 parts for 'periods' line, found ",
                           line_parts.size(), " in '", stripped_line, "'"));
        }
        try {
          ...
          return absl::InvalidArgumentError(
              absl::StrCat("Invalid number format for periods '", line_parts[1],
                           "': ", e.what()));
        }
...
absl::StatusOr<Eigen::VectorXd> GetCircuitCurrents(
  ...
  for (int i = 0; i < number_of_serial_circuits; ++i) {
    circuit_currents[i] =
        magnetic_configuration.serial_circuits(i).current_.value();
  }
...
absl::Status SetCircuitCurrents(
  ...
  if (number_of_serial_circuits != number_of_circuit_currents) {
    return absl::InvalidArgumentError(absl::StrCat(
        ...
        ") does not match number of SerialCircuits (",
        number_of_serial_circuits, ") in MagneticConfiguration."));
  }

  for (int i = 0; i < number_of_serial_circuits; ++i) {
    // Get mutable pointer and set optional member directly
    m_magnetic_configuration.mutable_serial_circuits(i)->current_ =
        circuit_currents[i];
  }
  ...
  for (int idx_circuit = 0; idx_circuit < num_serial_circuits; ++idx_circuit) {
    ...
    for (int idx_coil = 0; idx_coil < num_coils; ++idx_coil) {
      ...
      if (idx_coil == 0) {
        ...
        return absl::InvalidArgumentError(absl::StrCat(
            "not all num_windings are |equal| in coil: |",
            coil.num_windings_.value(), "| =!= |", unique_num_windings, "|"));
      }
...
absl::Status MoveRadially(double radial_step,
  ...
  if (normal.x() != 0.0 || normal.y() != 0.0 || std::abs(normal.z()) < 1e-9) {
    return absl::InvalidArgumentError(
        "Normal must be aligned with the z-axis (x=0, y=0) for this radial "
        "movement");
  }

  // Modify the radius directly (dereference, modify, assign back)
  m_circular_filament.radius += radial_step;
  // Optionally, check if the new radius is non-negative
  if (m_circular_filament.radius < 0.0) {
    // Revert change and return error? Or clamp? Let's error.
    m_circular_filament.radius -= radial_step;  // Revert
    return absl::InvalidArgumentError(
        "Radial step results in negative radius.");
  }
...
absl::Status MoveRadially(double radial_step,
  ...
  for (int i = 0; i < m_polygon_filament.vertices.cols(); ++i) {
    Eigen::Vector3d vertex = m_polygon_filament.vertices.col(
        i);  // Extract the vertex as a column vector

    const double r_squared = vertex.x() * vertex.x() + vertex.y() * vertex.y();
    ...
    m_polygon_filament.vertices.col(i) =
        vertex;  // Update the vertex in the matrix
  }
  return absl::OkStatus();
}  // MoveRadially
...
absl::Status MoveRadially(double radial_step,
  ...
  for (int idx_circuit = 0;
       idx_circuit < m_magnetic_configuration.serial_circuits_size();
       ++idx_circuit) {
    SerialCircuit* m_serial_circuit =
        m_magnetic_configuration.mutable_serial_circuits(idx_circuit);
    for (int idx_coil = 0; idx_coil < m_serial_circuit->coils_size();
         ++idx_coil) {
      Coil* m_coil = m_serial_circuit->mutable_coils(idx_coil);
      for (int idx_current_carrier = 0;
           idx_current_carrier < m_coil->current_carriers_size();
           ++idx_current_carrier) {
        ...
        switch (m_current_carrier->type_case()) {
          case CurrentCarrier::TypeCase::kInfiniteStraightFilament:
            return absl::InvalidArgumentError(
                "Cannot perform radial movement on an "
          ...
          case CurrentCarrier::TypeCase::kCircularFilament:
            ...
            status = MoveRadially(
                radial_step, *(m_current_carrier->mutable_circular_filament()));
          ...
          default:
            return absl::InternalError(
                absl::StrCat("Unsupported current carrier type encountered "
                             "during radial move: ",
...
// ------------------
// Helper functions to create identifiers (use direct optional check and
// dereference)
...
std::string CurrentCarrierIdentifier(
  ...
  if (inf_filament.name) {                            // Check optional directly
    ss << " (name: '" << *inf_filament.name << "')";  // Dereference
  }
  return ss.str();
}
...
std::string CurrentCarrierIdentifier(const CircularFilament& circ_filament) {
  std::stringstream ss;
  ss << "CircularFilament";
  if (circ_filament.name) {
    ss << " (name: '" << *circ_filament.name << "')";
  }
  return ss.str();
}
...
std::string CurrentCarrierIdentifier(const PolygonFilament& poly_filament) {
  std::stringstream ss;
  ss << "PolygonFilament";
  if (poly_filament.name) {
    ss << " (name: '" << *poly_filament.name << "')";
  }
  return ss.str();
}
...
absl::Status IsInfiniteStraightFilamentFullyPopulated(
  ...
  if (!inf_filament.origin_) {
    return absl::FailedPreconditionError(
        absl::StrCat(CurrentCarrierIdentifier(inf_filament),
  ...
  if (!inf_filament.direction_) {
    ...
    if (std::abs(dir.x()) < 1e-9 && std::abs(dir.y()) < 1e-9 &&
        std::abs(dir.z()) < 1e-9) {
      return absl::InvalidArgumentError(
          absl::StrCat(CurrentCarrierIdentifier(inf_filament),
                       " has zero direction vector."));
    }
  }
...
absl::Status IsCircularFilamentFullyPopulated(
  ...
  // center, normal, and radius are now non-optional plain fields,
  // so they are always populated after construction.
  return absl::OkStatus();
}

absl::Status IsPolygonFilamentFullyPopulated(
    const PolygonFilament& polygon_filament) {
  if (polygon_filament.vertices.cols() < 2) {
    std::stringstream error_message;
    error_message << CurrentCarrierIdentifier(polygon_filament);
    error_message << " has too few vertices ("
                  << polygon_filament.vertices.cols() << "); need at least 2.";
...
absl::Status IsMagneticConfigurationFullyPopulated(
  ...
  if (!magnetic_configuration.num_field_periods_) {
    return absl::FailedPreconditionError(
        "MagneticConfiguration requires 'num_field_periods'.");
  ...
  for (int idx_circuit = 0;
       idx_circuit < magnetic_configuration.serial_circuits_size();
       ++idx_circuit) {
    const SerialCircuit& serial_circuit =
        magnetic_configuration.serial_circuits(idx_circuit);

    // Check circuit fields directly
    if (!serial_circuit.current_) {
      return absl::FailedPreconditionError(absl::StrCat(
          "SerialCircuit index ", idx_circuit,
          (serial_circuit.name ? absl::StrCat(" ('", *serial_circuit.name, "')")
                               : ""),
          " requires 'current'."));
    }
    ...
    for (int idx_coil = 0; idx_coil < serial_circuit.coils_size(); ++idx_coil) {
      ...
      if (!coil.num_windings_) {
        return absl::FailedPreconditionError(absl::StrCat(
            ...
            (coil.name ? absl::StrCat(" ('", *coil.name, "')") : ""),
            " requires 'num_windings'."));
      }

      for (int idx_carrier = 0; idx_carrier < coil.current_carriers_size();
           ++idx_carrier) {
        const CurrentCarrier& current_carrier =
            coil.current_carriers(idx_carrier);
        absl::Status status = absl::OkStatus();
        ...
        switch (current_carrier.type_case()) {
          ...
          case CurrentCarrier::TypeCase::kFourierFilament:
            status = absl::UnimplementedError(
                "Population check for FourierFilament not implemented.");
            break;
          case CurrentCarrier::TypeCase::kTypeNotSet:
            status = absl::FailedPreconditionError(
                absl::StrCat("CurrentCarrier index ", idx_carrier,
                             " in Coil index ", idx_coil, " in Circuit index ",
                             idx_circuit, " has kTypeNotSet."));
            break;
          default:
...
void PrintInfiniteStraightFilament(const InfiniteStraightFilament& inf_filament,
                                   int indentation) {
  ...
  std::cout << prefix << "  name: "
            << (inf_filament.name ? absl::StrCat("'", *inf_filament.name, "'")
                                  : "[not set]")
            << '\n';
  if (inf_filament.origin_) {
...
void PrintCircularFilament(const CircularFilament& circ_filament,
                           int indentation) {
  std::string prefix(indentation, ' ');
  std::cout << prefix << "CircularFilament {" << '\n';
  std::cout << prefix << "  name: "
            << (circ_filament.name ? absl::StrCat("'", *circ_filament.name, "'")
                                   : "[not set]")
            << '\n';
  std::cout << prefix << "  center: [" << circ_filament.center.x() << ", "
            << circ_filament.center.y() << ", " << circ_filament.center.z()
            << "]\n";

  const Vector3d& normal = circ_filament.normal;
  std::cout << prefix << "  normal: [" << normal.x() << ", " << normal.y()
            << ", " << normal.z() << "]\n";
  std::cout << prefix << "  radius: " << circ_filament.radius << '\n';
  std::cout << prefix << "}" << '\n';
}
...
void PrintPolygonFilament(const PolygonFilament& polygon_filament,
  ...
  if (polygon_filament.vertices.cols() > 0) {
    std::cout << prefix << "  vertices: [" << polygon_filament.vertices.cols()
              << "]" << '\n';
  } else {
...
void PrintSerialCircuit(const SerialCircuit& serial_circuit, int indentation) {
  ...
  std::cout << prefix << "  current: " << serial_circuit.current_ << '\n';
