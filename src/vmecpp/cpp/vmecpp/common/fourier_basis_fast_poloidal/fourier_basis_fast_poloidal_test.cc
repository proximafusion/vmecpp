...
TEST(TestFourierBasisFastPoloidal, CheckCos2CCSS) {
  ...
  for (int m = 0; m < s.mpol; ++m) {
    // test n = 0 separately: only one contrib from fcCC
    {
      ...
      const double basis_norm = fb.mscale[m] * fb.nscale[n];

      const int mn_pos = fb.mnIdx(m, n);
      const double to_test = fc_cc[m * (s.ntor + 1) + n] * basis_norm;
    ...
    for (int n = 1; n < s.ntor + 1; ++n) {
...

      {
        // test +n
        const int mn_pos = fb.mnIdx(m, n);
        double to_test = fc_cc[m * (s.ntor + 1) + n] * basis_norm;
        if (m > 0) {
          to_test += fc_ss[m * (s.ntor + 1) + n] * basis_norm;
          to_test /= 2.0;
        }
      ...
      if (m > 0) {
        ...
        // test -n
        const int mn_neg = fb.mnIdx(m, -n);
        double to_test =
            0.5 * (fc_cc[m * (s.ntor + 1) + n] - fc_ss[m * (s.ntor + 1) + n]) *
            basis_norm;
...
TEST(TestFourierBasisFastPoloidal, CheckSin2SCCS) {
  ...
  for (int m = 0; m < s.mpol; ++m) {
    ...
    if (m == 0) {
      ...
      EXPECT_TRUE(IsCloseRelAbs(0.0, fc_sc[m * (s.ntor + 1) + n], kTolerance))
      ...
      double to_test = fc_sc[m * (s.ntor + 1) + n] * basis_norm;
    ...
    for (n = 1; n < s.ntor + 1; ++n) {
      ...
      if (m == 0) {
        // test m = 0 separately: only one contrib from fcCS
        {
          // test +n
          int mn_pos = fb.mnIdx(m, n);
          double to_test = -fc_cs[m * (s.ntor + 1) + n] * basis_norm;
          EXPECT_TRUE(IsCloseRelAbs(fc_sin_in[mn_pos], to_test, kTolerance))
              << absl::StrFormat("m=%d n=%d", m, n);
        }
      } else {
        {
          ...
          int mn_pos = fb.mnIdx(m, n);
          double to_test =
              0.5 * (fc_sc[m * (s.ntor + 1) + n] - fc_cs[m * (s.ntor + 1) + n]) *
              basis_norm;
          EXPECT_TRUE(IsCloseRelAbs(fc_sin_in[mn_pos], to_test, kTolerance))
              << absl::StrFormat("m=%d n=%d", m, n);
        }
  ...
  for (int m = 0; m < s.mpol; ++m) {
    for (int k = 0; k < s.nZeta; ++k) {
      ...
      for (int n = 0; n < s.ntor + 1; ++n) {
        ...
        rnkcc += fc_cc[idx_mn] * cosnv;
        rnkss += fc_ss[idx_mn] * sinnv;
      }
  ...
  for (int m = 0; m < s.mpol; ++m) {
    for (int k = 0; k < s.nZeta; ++k) {
      ...
      for (int n = 0; n < s.ntor + 1; ++n) {
        ...
        rnksc += fc_sc[idx_mn] * cosnv;
        rnkcs += fc_cs[idx_mn] * sinnv;
      }
...
TEST(TestFourierBasisFastPoloidal, CheckInvDFTCombined) {
  ...
  for (int k = 0; k < s.nZeta; ++k) {
    ...
    for (int l = 0; l < s.nThetaReduced; ++l) {
      ...
      int target_kl_reversed = k_reversed * s.nThetaEven + l_reversed;
...
TEST(TestFourierBasisFastPoloidal, CheckOrthogonality) {
  ...
  for (int k = 0; k < s.nZeta; ++k) {
    ...
    for (int l = 0; l < s.nThetaReduced; ++l) {
      ...
      int source_kl_reversed = k_reversed * s.nThetaEven + l_reversed;
  ...
  for (int k = 0; k < s.nZeta; ++k) {
    for (int m = 0; m < s.mpol; ++m) {
      ...
      for (int n = 0; n < s.ntor + 1; ++n) {
        ...
        fc_cc[idx_mn] += rnkcc * cosnv;
        fc_ss[idx_mn] += rnkss * sinnv;
        fc_sc[idx_mn] += rnksc * cosnv;
        fc_cs[idx_mn] += rnkcs * sinnv;
      }
    }
  }
