...
class NestorAsyncWorker {
 public:
  ...
  struct Outputs {
    ...
    double bSubVVac = 0.0;
    bool ok = true;
    // The vacuum solve's status; a boundary outside the vacuum grid reports
    // here and the consumer folds it into HandoverStorage::vacuum_status.
    absl::Status status;
  };
 ...
 private:
  ...
  void Compute(const Inputs& in, Outputs& result) {
    ...
    double bsubu = 0.0;
    double bsubv = 0.0;
    const absl::StatusOr<bool> rc = fb_->update(
        sp(in.rCC), sp(in.rSS), sp(in.rSC), sp(in.rCS), sp(in.zSC), sp(in.zCS),
        sp(in.zCC), sp(in.zSS), in.signOfJacobian, sp(in.rAxis), sp(in.zAxis),
        &bsubu, &bsubv, in.netToroidalCurrent, in.ivacskip);
