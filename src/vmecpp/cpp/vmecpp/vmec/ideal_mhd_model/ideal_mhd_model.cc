  ...
  for (int jF = rp.nsMinF; jF < jMaxRZ; ++jF) {
    ...
    for (int m = 0; m < mmax; ++m) {
      ...
      for (int k = 0; k < s.nZeta; ++k) {
        ...
        rmkcc_n = -crmn_seg.dot(cosmui_seg);
        zmkcs_n = -czmn_seg.dot(cosmui_seg);

        rmkss_n = -crmn_seg.dot(sinmui_seg);
        zmksc_n = -czmn_seg.dot(sinmui_seg);
        ...
        rmkcc = tempR_seg.dot(cosmui_seg) + brmn_seg.dot(sinmumi_seg);
        rmkss = tempR_seg.dot(sinmui_seg) + brmn_seg.dot(cosmumi_seg);
        zmksc = tempZ_seg.dot(sinmui_seg) + bzmn_seg.dot(cosmumi_seg);
        zmkcs = tempZ_seg.dot(cosmui_seg) + bzmn_seg.dot(sinmumi_seg);
  ...
  for (int jF = nsMinF1; jF < r.nsMaxF1; ++jF) {
    for (int m = 0; m < s.mpol; ++m) {
      ...
      for (int k = 0; k < s.nZeta; ++k) {
        ...
        rmkcc = rmncc_seg.dot(cosnv_seg);
        rmkcc_n = rmncc_seg.dot(sinnvn_seg);
        rmkss = rmnss_seg.dot(sinnv_seg);
        rmkss_n = rmnss_seg.dot(cosnvn_seg);
        zmksc = zmnsc_seg.dot(cosnv_seg);
        zmksc_n = zmnsc_seg.dot(sinnvn_seg);
        zmkcs = zmncs_seg.dot(sinnv_seg);
        zmkcs_n = zmncs_seg.dot(cosnvn_seg);
        lmksc = lmnsc_seg.dot(cosnv_seg);
        lmksc_n = lmnsc_seg.dot(sinnvn_seg);
        lmkcs = lmncs_seg.dot(sinnv_seg);
        lmkcs_n = lmncs_seg.dot(cosnvn_seg);
