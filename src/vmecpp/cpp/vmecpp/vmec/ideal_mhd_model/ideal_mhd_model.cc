...
absl::StatusOr<bool> IdealMhdModel::update(
...
#ifdef VMECPP_USE_CUDA
  // The device path applies zeroZForceForM1 unconditionally (see
  // DecomposeAndConstrainCuda below), which subsumes always_fix_m1_gauge.
  (void)always_fix_m1_gauge;
  ...
  if (iter2 >= 2 || device_x_authoritative_iter1) {
    RecomposeToPhysicalCuda(
        r_, s_, m_fc_, signOfJacobian, m_p_.scalxc, m_decomposed_x.rmncc.data(),
        m_decomposed_x.rmnss.data(), m_decomposed_x.zmnsc.data(),
        m_decomposed_x.zmncs.data(), m_decomposed_x.lmnsc.data(),
        m_decomposed_x.lmncs.data());
...
#pragma omp barrier
    ...
    if (m_vacuum_pressure_state_ != VacuumPressureState::kOff) {
      ...
      if (async_steady) {
        ...
        if (!prev.status.ok() && m_h_.vacuum_status.ok()) {
          m_h_.vacuum_status = prev.status;
        }
        ...
        for (int vac_cfg = 0; vac_cfg < n_vac_cfg; ++vac_cfg) {
          // Under the CUDA build the vacuum solve is not partitioned
          // (vac_num_threads_ == 1; see Vmec::SetupVacuumSolvers), so
          // (*m_fb_vac_)[0] is the full-tangential-grid solver.
          FreeBoundaryBase* fb_cfg = fb_per_cfg_.empty() ? (*m_fb_vac_)[0].get()
                                                         : fb_per_cfg_[vac_cfg];
          ...
          const absl::StatusOr<bool> rc = fb_cfg->update(
              ...
              iter2 >= iterations_before_checkpointing);
          // A boundary may leave the vacuum grid transiently while the
          // equilibrium is still moving; the first error is kept and
          // Vmec::run turns a still-outside boundary into an error once the
          // run has converged.
          if (!rc.ok()) {
            ...
            if (verbose) {
              std::cout << "WARNING: " << rc.status().message() << "\n";
            }
          } else if (*rc) {
            return true;
          }
...
#pragma omp single
  ...
  // (contain cuFFT, regress in captured-graph mode).
  {
    ...
    // lthreed iteration, where the CPU path gates it on
    // always_fix_m1_gauge, fsqz < 1e-6, or iter2 < 2; the unconditional
    // form holds the m = 1 constraint exactly throughout and leaves the
    // convergence direction unchanged.
    DecomposeAndConstrainCuda(
        r_, s_, m_fc_, 1.0 / std::numbers::sqrt2, signOfJacobian, m_p_.scalxc,
    ...
    // orthogonal: the change of variables leaves the residual norm that fsqr
    // and fsqz measure unchanged, and is its own inverse. With 1/2 the map
    // would instead halve the residuals on every application. Fortran
    // residue.f90 constrain_m1 uses osqrt2 for the same reason.
...
void IdealMhdModel::computePreconditioningMatrix(
...
#ifdef VMECPP_USE_CUDA
  {
    ...
    // side one). The lforbal force-balance weight (trigmult, m_eqfactor)
    // is not computed on the device; Vmec::run rejects lforbal inputs
    // under the CUDA build before the iteration starts.
    (void)trigmult;
    (void)m_eqfactor;
