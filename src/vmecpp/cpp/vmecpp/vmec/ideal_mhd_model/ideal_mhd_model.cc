...
void IdealMhdModel::geometryFromFourier(const FourierGeometry& physical_x) {
  ...
  if (s_.lasym) {
    // CRITICAL: Apply M=1 constraints every iteration before transforms (jVMEC
    // requirement)
    FourierGeometry& mutable_physical_x =
        const_cast<FourierGeometry&>(physical_x);

    if (s_.lthreed) {
      // Apply M=1 constraint to 3D symmetric arrays: rmnss <-> zmncs
      EnsureM1Constrained(s_, absl::MakeSpan(mutable_physical_x.rmnss),
    ...
    EnsureM1Constrained(s_, absl::MakeSpan(mutable_physical_x.rmnsc),
    ...
    // NOTE: The non-separated asymmetric transforms already handle
    // symmetrization internally, so we don't need to call
    // symmetrizeRealSpaceGeometry() here
  }  // lasym
...
void IdealMhdModel::dft_FourierToReal_3d_asymm(
  ...
  // The FourierGeometry arrays are indexed from physical_x.nsMin(), not from
  // r_.nsMinF We need to adjust the offset accordingly
  ...
  const auto lmncc =
      physical_x.lmncc.subspan(offset, count);  // CRITICAL: asymmetric
  const auto lmnss =
      physical_x.lmnss.subspan(offset, count);  // CRITICAL: asymmetric
  ...
  SymmetrizeRealSpaceGeometry(
      ...
      absl::MakeSpan(r1_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      absl::MakeSpan(z1_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      absl::MakeSpan(lu_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      s_);

  // CRITICAL: Compute spectral condensation for asymmetric modes (missing
  // piece!) This must be done after symmetrization to include all theta values
  for (int jF = r_.nsMinF; jF < r_.nsMaxFIncludingLcfs; ++jF) {
    ...
    for (int m = 2; m < s_.mpol - 1; ++m) {
      ...
      for (int n = 0; n <= s_.ntor; ++n) {
        ...
        const double rmkcs =
            (imn < rmncs.size() && s_.lthreed) ? rmncs[imn] : 0.0;
        const double zmkcc = (imn < zmncc.size()) ? zmncc[imn] : 0.0;
        const double zmkss =
            (imn < zmnss.size() && s_.lthreed) ? zmnss[imn] : 0.0;
...
void IdealMhdModel::dft_FourierToReal_2d_asymm(
  ...
  auto rmnsc = physical_x.rmnsc.empty()
                   ? absl::Span<const double>()
                   : physical_x.rmnsc.subspan(offset, count);
  auto rmncs = physical_x.rmncs.empty()
                   ? absl::Span<const double>()
                   : physical_x.rmncs.subspan(offset, count);
  auto zmncc = physical_x.zmncc.empty()
                   ? absl::Span<const double>()
                   : physical_x.zmncc.subspan(offset, count);
  auto zmnss = physical_x.zmnss.empty()
                   ? absl::Span<const double>()
                   : physical_x.zmnss.subspan(offset, count);

  auto rmncc = physical_x.rmncc.empty()
                   ? absl::Span<const double>()
                   : physical_x.rmncc.subspan(offset, count);
  auto rmnss = physical_x.rmnss.empty()
                   ? absl::Span<const double>()
                   : physical_x.rmnss.subspan(offset, count);
  auto zmnsc = physical_x.zmnsc.empty()
                   ? absl::Span<const double>()
                   : physical_x.zmnsc.subspan(offset, count);
  auto zmncs = physical_x.zmncs.empty()
                   ? absl::Span<const double>()
                   : physical_x.zmncs.subspan(offset, count);

  // Lambda coefficients (both symmetric and asymmetric)
  auto lmnsc = physical_x.lmnsc.empty()
                   ? absl::Span<const double>()
                   : physical_x.lmnsc.subspan(offset, count);
  auto lmncs = physical_x.lmncs.empty()
                   ? absl::Span<const double>()
                   : physical_x.lmncs.subspan(offset, count);
  auto lmncc =
      physical_x.lmncc.empty()
          ? absl::Span<const double>()
          : physical_x.lmncc.subspan(offset, count);  // CRITICAL: asymmetric
  auto lmnss =
      physical_x.lmnss.empty()
          ? absl::Span<const double>()
          : physical_x.lmnss.subspan(offset, count);  // CRITICAL: asymmetric
  ...
  FourierToReal2DAsymmFastPoloidal(
      s_, rmncc, rmnss, rmnsc, rmncs, zmnsc, zmncs, zmncc, zmnss, lmnsc, lmncs,
      lmncc, lmnss,  // CRITICAL: Lambda coefficients added
      absl::MakeSpan(r1_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      absl::MakeSpan(z1_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      absl::MakeSpan(lu_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      absl::MakeSpan(ru_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp),
      absl::MakeSpan(zu_e).subspan((r_.nsMinF - r_.nsMinF1) * s_.nZnT,
                                   num_realsp));
}
...
void IdealMhdModel::dft_ForcesToFourier_3d_asymm(FourierForces& m_physical_f) {
  ...
  RealToFourier3DAsymmFastPoloidal(
      s_, absl::MakeConstSpan(armn_e), absl::MakeConstSpan(azmn_e),
      absl::MakeConstSpan(blmn_e), absl::MakeSpan(m_physical_f.frcc),
      absl::MakeSpan(m_physical_f.frss),
      absl::MakeSpan(m_physical_f.frsc),  // Asymmetric
      absl::MakeSpan(m_physical_f.frcs),  // Asymmetric
      absl::MakeSpan(m_physical_f.fzsc), absl::MakeSpan(m_physical_f.fzcs),
      absl::MakeSpan(m_physical_f.fzcc),  // Asymmetric
      absl::MakeSpan(m_physical_f.fzss),  // Asymmetric
      absl::MakeSpan(m_physical_f.flsc), absl::MakeSpan(m_physical_f.flcs),
...
void IdealMhdModel::dft_ForcesToFourier_2d_asymm(FourierForces& m_physical_f) {
  ...
  RealToFourier2DAsymmFastPoloidal(
      s_, absl::MakeConstSpan(armn_e), absl::MakeConstSpan(azmn_e),
      absl::MakeConstSpan(blmn_e), absl::MakeSpan(m_physical_f.frcc),
      absl::MakeSpan(m_physical_f.frss), absl::MakeSpan(m_physical_f.frsc),
      absl::MakeSpan(m_physical_f.frcs), absl::MakeSpan(m_physical_f.fzsc),
      absl::MakeSpan(m_physical_f.fzcs), absl::MakeSpan(m_physical_f.fzcc),
      absl::MakeSpan(m_physical_f.fzss), absl::MakeSpan(m_physical_f.flsc),
      absl::MakeSpan(m_physical_f.flcs), absl::MakeSpan(m_physical_f.flcc),
...
void IdealMhdModel::symmetrizeForces() {
  ...
  SymmetrizeForces(s_, absl::MakeSpan(armn_e).subspan(0, num_realsp),
