...
void vmecpp::OutputQuantities::Rescale(double b_scale, double r_scale,
                                       bool scale_pressure) {
  // 1. Scale INDATA parameters
  indata.phiedge *= b_scale * r_scale * r_scale;
  if (scale_pressure) {
    indata.pres_scale *= b_scale * b_scale;
  }
  indata.curtor *= b_scale * r_scale;

  // boundary coefficients
  indata.rbc *= r_scale;
  indata.zbs *= r_scale;
  if (indata.rbs.has_value()) {
    indata.rbs.value() *= r_scale;
  }
  if (indata.zbc.has_value()) {
    indata.zbc.value() *= r_scale;
  }

  // magnetic axis guess
  indata.raxis_c *= r_scale;
  indata.zaxis_s *= r_scale;
  if (indata.raxis_s.has_value()) {
    indata.raxis_s.value() *= r_scale;
  }
  if (indata.zaxis_c.has_value()) {
    indata.zaxis_c.value() *= r_scale;
  }

  // 2. Scale WOUT geometry
  // Vmec's InitFromState only requires rmnc, zmns, and lmns_full.
  // For lasym, it might need rmns, zmnc as well in the future.
  wout.rmnc *= r_scale;
  wout.zmns *= r_scale;
  wout.rmns *= r_scale;
  wout.zmnc *= r_scale;
