...
absl::Status vmecpp::WOutFileContents::WriteTo(H5::H5File& file) const {
  ...
  WRITEMEMBER(overr);
  WRITEMEMBER(jdotb);
  //TODO(jurasic) We will deprecate HDF5 soon, regenerate large_cpp_tests reference files with all quantities once that is done
  // WRITEMEMBER(bdotb);
  WRITEMEMBER(bdotgradv);
  WRITEMEMBER(DMerc);
  WRITEMEMBER(Dshear);
...
absl::Status vmecpp::WOutFileContents::LoadInto(WOutFileContents& obj,
  ...
  // Compatibility with HDF5 files that do not have the nextcur and extcur fields yet (before v0.3.3)
  if (obj.lfreeb){
    READMEMBER(nextcur);
    READMEMBER(extcur);
  }else{
