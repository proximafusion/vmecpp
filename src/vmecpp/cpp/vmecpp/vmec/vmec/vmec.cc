...
absl::StatusOr<bool> Vmec::run(const VmecCheckpoint& checkpoint,
  ...
  for (jacob_off_ = 0; jacob_off_ < 2; ++jacob_off_) {
    ...
    // instead of slowly activating it (as we would in a regular fb solve).
    // If the initial guess completely off, jacob_off>0 will
    // trigger a retry with VacuumPressureState::kOff anyway.
