...
absl::StatusOr<bool> Vmec::run(const VmecCheckpoint& checkpoint,
  ...
  if (indata_.lforbal) {
    return absl::UnimplementedError(
        "lforbal (the m = 1 force-balance modification) is not supported by "
        "the CUDA build; rebuild without VMECPP_USE_CUDA for lforbal inputs");
  }
...
void Vmec::SetupVacuumSolvers() {
  ...
  omp_set_nested(1);
#if _OPENMP >= 200805
  // OpenMP 3.0; the MSVC runtime implements OpenMP 2.0, where nesting is
  // enabled by omp_set_nested alone.
  ...
  // LU decomposition and reduction slots. They share the mgrid provider and
  // the HandoverStorage output spans; the vacuum loop in
  // IdealMhdModel::update consumes each configuration's outputs before the
  // next solver call overwrites them. Like fb_vac_, the instances persist
  // across multigrid stages. All spans captured here are sized in the Vmec
  // constructor (ns-independent).
  ...
  if (n_cfg_fb > 1 &&
    ...
    fb_lu_per_cfg_.resize(n_cfg_fb - 1);
    fb_bvec_per_cfg_.resize(n_cfg_fb - 1);
    fb_reduce_slots_per_cfg_.resize(n_cfg_fb - 1);
    ...
    for (int c = 0; c + 1 < n_cfg_fb; ++c) {
      ...
      // One tangential partition per instance, so the slots hold one
      // matrix-sized contribution (vacuum_reduce_slots_ with one thread).
      fb_reduce_slots_per_cfg_[c].setZero(mnpd * mnpd);
      fb_extra_cfg_[c] = std::make_unique<Nestor>(
          ...
          std::span<double>(h_.vacuum_b_phi.data(), h_.vacuum_b_phi.size()),
          std::span<double>(h_.vacuum_b_z.data(), h_.vacuum_b_z.size()),
          std::span<double>(fb_reduce_slots_per_cfg_[c].data(),
                            fb_reduce_slots_per_cfg_[c].size()));
    }
  }

  // Asynchronous NESTOR (single configuration): a thread-private Nestor
  // with its own response matrix, right-hand side, LU decomposition,
  // reduction slots, and field/pressure output buffers, so the worker
  // thread can run it concurrently with the device iteration without racing
  // the shared HandoverStorage. Persists across multigrid stages like
  // fb_vac_. The overlap is opt-in: IdealMhdModel::update uses the worker
  // only when VMECPP_FB_ASYNC_NESTOR is set to 1, so the instance is built
  // only then.
  const char* async_env = std::getenv("VMECPP_FB_ASYNC_NESTOR");
  const bool async_enabled = (async_env != nullptr && std::atoi(async_env) > 0);
  if (n_cfg_fb == 1 && async_enabled &&
      indata_.free_boundary_method == FreeBoundaryMethod::NESTOR) {
    const int nf = s_.ntor;
    const int mf = s_.mpol + 1;
    const int mnpd = (2 * nf + 1) * (mf + 1);
    ...
    fb_async_reduce_slots_.setZero(mnpd * mnpd);
    ...
    fb_async_ = std::make_unique<Nestor>(
        ...
        std::span<double>(fb_async_reduce_slots_.data(),
                          fb_async_reduce_slots_.size()));
  }
#endif  // VMECPP_USE_CUDA
}  // SetupVacuumSolvers
      ...
      if (n_cfg > 0 && static_cast<int>(fc_.fsqr_per_cfg.size()) == n_cfg &&
        ...
        for (int c = 0; c < n_cfg; ++c) {
          if (fc_.active_per_cfg[c]) {
            ...
            if (cfg_done || timed_out) {
              fc_.active_per_cfg[c] = 0;
              fc_.converged_per_cfg[c] = static_cast<std::uint8_t>(cfg_done);
#ifdef VMECPP_USE_CUDA
              // Freeze this cfg's state at the iteration it stopped; the
              ...
              if (!cfg_done) {
                std::fprintf(
                    ...
                    fc_.fsqr_per_cfg[c], fc_.fsqz_per_cfg[c],
                    fc_.fsql_per_cfg[c], fc_.ftolv);
              }
            } else {
              all_done = false;
