...
// protect read of m_liter_flag above from write below
...
#pragma omp single
  ...
  // `liter_loop`: FORCE ITERATION LOOP
  while (m_liter_flag) {
...
#pragma omp single
      ...
      if (iter2_ % indata_.nstep == 0 || iter2_ == 1 || !m_liter_flag) {
