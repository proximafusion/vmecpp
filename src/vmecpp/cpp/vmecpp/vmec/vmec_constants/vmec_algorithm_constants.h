...
// several undocumented scalings. The tail-convergence study
// (docs/convergence_study.md, Finding 10) measured that the slow eigenmodes
// of the preconditioned descent are dominated by low-m core lambda
// coefficients on all tested equilibria, so these scalings are the natural
// tuning hyperparameters for the tail rate; they are collected here to make
// them visible and adjustable in one place.
