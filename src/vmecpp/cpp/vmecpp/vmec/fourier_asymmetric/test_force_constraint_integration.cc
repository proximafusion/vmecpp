...
TEST_F(ForceConstraintIntegrationTest, ForceConstraintWithRealConfiguration) {
  ...
  std::cout << "RSS: " << original.frss_m1_n0
            << ", ZCS: " << original.fzcs_m1_n0 << "\n";
  std::cout << "RSC: " << original.frsc_m1_n0
            << ", ZCC: " << original.fzcc_m1_n0 << "\n\n";
  ...
  double constrained_symmetric_energy =
      constrained_rss * constrained_rss + constrained_zcs * constrained_zcs;
  double energy_ratio_symmetric =
      constrained_symmetric_energy / original_symmetric_energy;

  double original_asymmetric_energy =
      original.frsc_m1_n0 * original.frsc_m1_n0 +
      original.fzcc_m1_n0 * original.fzcc_m1_n0;
  double constrained_asymmetric_energy =
      constrained_rsc * constrained_rsc + constrained_zcc * constrained_zcc;
  double energy_ratio_asymmetric =
      constrained_asymmetric_energy / original_asymmetric_energy;
  ...
  std::cout << "Expected ratio for jVMEC constraint: "
            << (2.0 * force_scaling * force_scaling) << "\n\n";
...
TEST_F(ForceConstraintIntegrationTest, VerifyConstraintApplicationTiming) {
  ...
  std::cout
      << "4. 🎯 Apply m=1 constraint: applyM1ConstraintToForces() [NEW]\n";
  ...
  std::cout
      << "VMEC++ NEW: Force calc → Constraint → Symmetrization ✅ FIXED\n\n";

  std::cout << "INTEGRATION POINT VALIDATION:\n";
  std::cout << "Location: ideal_mhd_model.cc forcesToFourier() function\n";
  std::cout
      << "Line: After dft_ForcesToFourier_*d_asymm(), before symrzl_forces()\n";
...
TEST_F(ForceConstraintIntegrationTest, ValidateScalingFactorDifference) {
  ...
  std::cout
      << "❌ OLD: VMEC++ used 0.5 scaling for geometry during initialization\n";
  std::cout << "✅ NEW: VMEC++ uses " << jvmec_force_scaling
            << " scaling for forces during iteration\n";
...
TEST_F(ForceConstraintIntegrationTest, TestConstraintWithMultipleModes) {
  ...
  std::vector<double> fzcs_values = {0.04, 0.025, 0.015,
                                     0.008};  // ZCS(m=1,n=0:3)

  std::cout << "ORIGINAL FORCES (m=1 modes):\n";
  for (int n = 0; n <= ntor; ++n) {
    std::cout << "n=" << n << ": RSS=" << frss_values[n]
              << ", ZCS=" << fzcs_values[n] << "\n";
  }
  std::cout << "\n";
  ...
  for (int n = 0; n <= ntor; ++n) {
    ...
    double expected_error =
        2.0 * force_scaling * fzcs_values[n];  // Factor includes scaling
