...
TEST_F(ForceConstraintImplementationTest,
       ImplementForceConstraintDuringIteration) {
  // STEP 1: Unit test to implement jVMEC force constraint application
  // Location: Should be applied to forces during iteration, not geometry during
  // initialization
  ...
  std::cout
      << "Timing: After force calculation, before spectral condensation\n\n";

  // Simulate force arrays (simplified for testing)
  std::vector<double> force_rss = {0.1, 0.05, 0.02, 0.01};  // m=1 forces
  std::vector<double> force_zcs = {0.05, 0.02, 0.01,
                                   0.005};  // corresponding Z forces
  ...
  double original_energy =
      force_rss[0] * force_rss[0] + force_zcs[0] * force_zcs[0];
  double constrained_energy = constrained_rss[0] * constrained_rss[0] +
                              constrained_zcs[0] * constrained_zcs[0];
  ...
  // The constraint: rss_new = scaling*(rss_old + zcs_old), zcs_new =
  // scaling*(rss_old - zcs_old) Energy: |rss_new|^2 + |zcs_new|^2 = scaling^2 *
  // (|rss_old + zcs_old|^2 + |rss_old - zcs_old|^2)
  //                                   = scaling^2 * (2*|rss_old|^2 +
  //                                   2*|zcs_old|^2) = 2 * scaling^2 *
  //                                   original_energy
  double expected_energy_ratio = 2.0 * force_scaling * force_scaling;
  EXPECT_NEAR(constrained_energy / original_energy, expected_energy_ratio,
              1e-10);
}

TEST_F(ForceConstraintImplementationTest,
       ConstraintForceMultiplierCalculation) {
  ...
  std::cout << "Purpose: Dynamic scaling based on surface count and "
               "preconditioner norms\n";
  ...
  std::cout << "Surface factor: "
            << (1.0 +
                numSurfaces * (1.0 / 60.0 + numSurfaces / (200.0 * 120.0)))
            << "\n";
  std::cout << "r0scale factor: "
            << ((4.0 * r0scale * r0scale) * (4.0 * r0scale * r0scale)) << "\n";
  ...
  for (int j = 1; j < numSurfaces - 1; ++j) {
    ...
    double profile_value = std::min(std::abs(ard_value / ard_norm),
                                    std::abs(azd_value / azd_norm)) *
                           constraint_multiplier *
  ...
  constraint_force_profile[numSurfaces - 2] =
      0.5 * constraint_force_profile[numSurfaces - 3];
  ...
  EXPECT_EQ(constraint_force_profile[numSurfaces - 2],
            0.5 * constraint_force_profile[numSurfaces - 3]);
}
...
TEST_F(ForceConstraintImplementationTest, BandPassFilteringImplementation) {
  ...
  EXPECT_EQ(
      active_modes,
      mpol - 2);  // Should have mpol-2 active modes (exclude m=0 and m=mpol-1)
}
...
TEST_F(ForceConstraintImplementationTest, ConstraintSymmetrizationTiming) {
  ...
  std::cout
      << "jVMEC order: Force calculation -> Constraint -> Symmetrization\n";
  std::cout << "VMEC++ order: Force calculation -> Symmetrization -> (no "
               "constraint)\n";
  ...
  double force_reversed =
      0.008;  // Force at reversed theta position (theta -> 2π-theta)
  ...
  double jvmec_antisymmetric =
      0.5 * (constrained_normal - constrained_reversed);

  std::cout << "JVMEC APPROACH (constraint first):\n";
  std::cout << "After constraint: normal=" << constrained_normal
            << ", reversed=" << constrained_reversed << "\n";
  std::cout << "After symmetrization: symmetric=" << jvmec_symmetric
            << ", antisymmetric=" << jvmec_antisymmetric << "\n\n";
  ...
  std::cout << "After symmetrization: symmetric=" << symmetric_first
            << ", antisymmetric=" << antisymmetric_first << "\n";
  std::cout << "After constraint: symmetric=" << alt_symmetric
            << ", antisymmetric=" << alt_antisymmetric << "\n\n";

  // Compare results
  std::cout << "COMPARISON:\n";
  std::cout << "Symmetric component difference: "
            << std::abs(jvmec_symmetric - alt_symmetric) << "\n";
  std::cout << "Antisymmetric component difference: "
            << std::abs(jvmec_antisymmetric - alt_antisymmetric) << "\n\n";
...
TEST_F(ForceConstraintImplementationTest, IntegrationLocationPlanning) {
  ...
  std::cout << "Integration point: After force calculation, before spectral "
               "condensation\n";
  std::cout << "Approximate line: Around line 1800-2000 (force processing "
               "section)\n\n";

  std::cout << "REQUIRED MODIFICATIONS:\n";
  std::cout << "1. Add force constraint application function\n";
  std::cout << "2. Calculate constraint force multiplier from tcon0 and "
               "surface count\n";
  ...
  std::cout
      << "void applyForceConstraint(Forces& forces, const Sizes& sizes,\n";
  std::cout << "                         double tcon0, int numSurfaces,\n";
  std::cout
      << "                         const RadialPreconditioner& precond)\n\n";
