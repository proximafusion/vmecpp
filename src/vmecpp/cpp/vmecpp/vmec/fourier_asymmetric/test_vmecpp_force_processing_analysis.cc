...
TEST_F(VMECPPForceProcessingAnalysisTest, CompareWithJVMECForceProcessingSequence) {
    ...
    std::cout << "1. Line 1059: computeEffectiveConstraintForce() - compute constraint force in real space\n";
    std::cout << "2. Line 1064: deAliasConstraintForce() - band-pass filter (m=1 to mpol-2) in Fourier space\n";
    std::cout << "3. Line 1092: computeMHDForces() - compute standard MHD forces in real space\n";
    std::cout << "4. Line 1103: symmetrizeForces() - symmetrize forces (if lasym=true)\n";
    std::cout << "5. Line 1106: toFourierSpace() - transform all forces to Fourier space\n";
    std::cout << "6. Line 1113: getDecomposedForces() -> convert_to_m1_constrained() - apply 1/√2 constraint\n";
    std::cout << "7. Line 1125: applyPreconditionerForM1Constraint() - special m=1 preconditioner\n\n";
    
    std::cout << "VMEC++ FORCE PROCESSING SEQUENCE (ideal_mhd_model.cc):\n";
    std::cout << "1. Line ~1850: MHD force calculation in real space\n";
    std::cout << "2. Line ~1900: SymmetrizeForces() - force symmetrization (if lasym=true)\n";
    std::cout << "3. Line ~1950: RealToFourier transforms - transform forces to Fourier space\n";
    ...
    std::cout << "B. Force constraint application (jVMEC line 1113, State.java lines 525-528)\n";
...
TEST_F(VMECPPForceProcessingAnalysisTest, IdentifyVMECPPIntegrationPoints) {
    // Identify specific locations in VMEC++ code where jVMEC components should be added
    ...
    std::cout << "Location: After geometry calculation, before MHD force calculation\n";
    std::cout << "Estimated line: ~1800-1850\n";
    std::cout << "Function to add: computeEffectiveConstraintForce()\n";
    std::cout << "Purpose: Calculate constraint forces based on geometry derivatives\n\n";
    ...
    std::cout << "Purpose: Apply m=1 to m=(mpol-2) filtering to constraint forces\n\n";
    
    std::cout << "INTEGRATION POINT 3: Force Constraint Application\n";
    std::cout << "Location: After RealToFourier transform, before force decomposition\n";
    std::cout << "Estimated line: ~1950-2000\n";
    std::cout << "Function to add: applyM1ConstraintToForces()\n";
    std::cout << "Purpose: Apply 1/√2 constraint to force Fourier coefficients\n\n";
    ...
    std::cout << "Purpose: Special preconditioner scaling for m=1 constrained forces\n\n";
...
TEST_F(VMECPPForceProcessingAnalysisTest, ConstraintForceCalculationDetails) {
    ...
    std::cout << "JVMEC IMPLEMENTATION (SpectralCondensation.java lines 263-279):\n";
    std::cout << "Purpose: Calculate force contributions from spectral constraint\n";
    std::cout << "Formula: (R_con - R_con_0) * dRdTheta + (Z_con - Z_con_0) * dZdTheta\n";
    std::cout << "Components:\n";
    ...
    std::cout << "Impact: No constraint forces contribute to equilibrium equation\n";
    std::cout << "Required: Add constraint force calculation to ideal_mhd_model.cc\n\n";
    
    std::cout << "CONSTRAINT FORCE ALGORITHM:\n";
    std::cout << "1. Calculate R_con and Z_con from constrained geometry coefficients\n";
    std::cout << "2. Extrapolate R_con_0 and Z_con_0 into volume\n";
    std::cout << "3. Compute constraint force: F_constraint = (R_con - R_con_0) * dR/dtheta + (Z_con - Z_con_0) * dZ/dtheta\n";
    ...
    std::cout << "  (R_con[j][0][k][l] - R_con_0[j][k][l]) * dRdThetaCombined[j][k][l] +\n";
    std::cout << "  (Z_con[j][0][k][l] - Z_con_0[j][k][l]) * dZdThetaCombined[j][k][l]\n\n";
...
TEST_F(VMECPPForceProcessingAnalysisTest, M1ConstraintTimingAnalysis) {
    ...
    std::cout << "if (lthreed) {\n";
    std::cout << "    convert_to_m1_constrained(scaledState.frss, scaledState.fzcs, scalingFactor);\n";
    std::cout << "}\n";
    std::cout << "if (lasym) {\n";
    std::cout << "    convert_to_m1_constrained(scaledState.frsc, scaledState.fzcc, scalingFactor);\n";
    std::cout << "}\n\n";
    ...
    std::cout << "Required arrays: frss, fzcs (symmetric), frsc, fzcc (asymmetric)\n";
    std::cout << "Integration point: Before decomposeInto() call in ideal_mhd_model.cc\n\n";
    ...
    std::cout << "- Force coefficients redistributed according to m=1 constraint\n";
    std::cout << "- Total force energy scaled by factor of 1/2 (due to 1/√2 scaling)\n";
...
TEST_F(VMECPPForceProcessingAnalysisTest, ImplementationPriorityRanking) {
    ...
    std::cout << "Expected improvement: Should resolve main convergence difference\n\n";
    ...
    std::cout << "Impact: MEDIUM - Affects which modes contribute to constraint\n";
    std::cout << "Complexity: HIGH - Complex Fourier space filtering\n";
    std::cout << "Lines of code: ~100-150\n";
    std::cout << "Expected improvement: Prevents spurious high-m contributions\n\n";
    ...
    std::cout << "Expected improvement: Additional constraint enforcement mechanism\n\n";
    
    std::cout << "RECOMMENDED IMPLEMENTATION ORDER:\n";
    std::cout << "1. Start with Priority 1 (Force Constraint Application) - highest impact/complexity ratio\n";
    std::cout << "2. Test convergence improvement with minimal implementation\n";
    std::cout << "3. Add Priority 2 (Constraint Force Multiplier) if needed for fine-tuning\n";
    std::cout << "4. Consider Priority 3 and 4 only if convergence issues persist\n\n";
