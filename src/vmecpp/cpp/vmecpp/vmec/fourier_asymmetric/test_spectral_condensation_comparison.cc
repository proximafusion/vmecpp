...
TEST_F(SpectralCondensationComparisonTest, JVMECConvertToM1Constrained) {
  // jVMEC convert_to_m1_constrained algorithm from SpectralCondensation.java
  // lines 131-134: final double backup = rss_rsc[j][n][m]; rss_rsc[j][n][m] =
  // scalingFactor * (backup + zcs_zcc[j][n][m]); zcs_zcc[j][n][m] =
  // scalingFactor * (backup - zcs_zcc[j][n][m]);

  std::cout << "\nJVMEC CONVERT_TO_M1_CONSTRAINED ALGORITHM:\n";
  std::cout << "Purpose: Impose m=1 constraint for theta angle invariance\n";
  std::cout << "Constraint: RSS(n) = ZCS(n) for symmetric, RSC(n) = ZCC(n) for "
               "asymmetric\n\n";
  ...
  std::cout << "Constrained RSS: " << constrained_rss
            << ", ZCS: " << constrained_zcs << "\n";
  std::cout << "Verification: RSS - ZCS = "
  ...
  double scaling_factor_asymmetric =
      1.0 / std::sqrt(2.0);  // Force scaling factor
  ...
  std::cout << "Constrained RSC: " << constrained_rsc
            << ", ZCC: " << constrained_zcc << "\n";
  std::cout << "Verification: RSC - ZCC = "
...
TEST_F(SpectralCondensationComparisonTest, JVMECForceScalingFactors) {
  ...
  // 2. Force decomposition: scaling_factor = 1.0/sqrt(2.0) (in force
  // processing)

  std::cout << "JVMEC SCALING FACTOR USAGE:\n";
  std::cout << "1. Geometry spectral width calculation: 1.0\n";
  std::cout << "2. Force decomposition and constraint: 1.0/sqrt(2.0) = "
            << (1.0 / std::sqrt(2.0)) << "\n";
  ...
  std::cout << "Force scaling preserves constraint energy: "
            << (force_scaling * force_scaling * 2.0) << " = 1.0\n";
  std::cout << "Geometry scaling maintains coefficients: " << geometry_scaling
            << " = 1.0\n\n";
...
TEST_F(SpectralCondensationComparisonTest, VMECPPSpectralCondensationLocation) {
  ...
  std::cout
      << "Location: ideal_mhd_model.cc (spectral condensation functions)\n";
  std::cout << "Current implementation: m1Constraint() in fourier_geometry.cc "
               "line 225\n";
  ...
  std::cout << "2. jVMEC applies constraint in force decomposition vs VMEC++ "
               "in geometry\n";
  std::cout
      << "3. jVMEC has separate constraint force multiplier calculation\n";
...
TEST_F(SpectralCondensationComparisonTest, ConstraintForceMultiplierAnalysis) {
  ...
  double profile_value =
      std::min(std::abs(ard_value / ard_norm), std::abs(azd_value / azd_norm)) *
      constraint_multiplier * (32.0 / (numSurfaces - 1.0)) *
      (32.0 / (numSurfaces - 1.0));

  std::cout << "Profile constraint value: " << profile_value << "\n";
  std::cout << "Surface scaling: "
...
TEST_F(SpectralCondensationComparisonTest, AsymmetricConstraintDifferences) {
  ...
  std::cout << "1. Uses convert_to_m1_constrained for both symmetric and "
               "asymmetric\n";
  std::cout << "2. Different scaling factors for forces vs geometry\n";
  std::cout
      << "3. Constraint force profile calculation includes asymmetric factor\n";
  ...
  std::cout
      << "A. Force decomposition vs geometry constraint application timing\n";
