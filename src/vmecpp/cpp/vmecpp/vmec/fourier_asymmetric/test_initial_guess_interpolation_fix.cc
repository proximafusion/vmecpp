...
TEST(InitialGuessInterpolation, AsymmetricModeInterpolation) {
  ...
  for (int js = 1; js <= ns; ++js) {
    ...
    std::cout << "js=" << js << ", sqrts=" << sqrts_js << ", s=" << s_js
              << std::endl;
    ...
    // Test m=0 mode (axis contribution)
    {
      ...
      std::cout << "  m=0: boundary_contrib=" << boundary_contrib
                << ", axis_contrib=" << axis_contrib << ", total=" << total
                << std::endl;
...
TEST(InitialGuessInterpolation, AsymmetricPerturbationCase) {
  ...
  for (int js = 1; js <= ns; ++js) {
    ...
    double r_m0 = sqrts_js * sqrts_js * rbc_00 +
                  (1.0 - sqrts_js * sqrts_js) * 1.0;  // Assuming axis at R=1
    ...
    std::cout << "js=" << js << ": R_m0=" << r_m0 << ", R_m1_cos=" << r_m1_cos
              << ", R_m1_sin=" << r_m1_sin << ", Z_m1_sin=" << z_m1_sin
              << std::endl;
