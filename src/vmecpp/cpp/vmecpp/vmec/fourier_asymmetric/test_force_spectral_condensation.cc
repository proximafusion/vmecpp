...
TEST_F(ForceSpectralCondensationTest, JVMECForceConstraintApplication) {
    ...
    std::cout << "Application: Forces during iteration, not geometry initialization\n";
    std::cout << "Timing: Called in force decomposition and spectral width calculation\n";
    std::cout << "Scaling: 1.0/sqrt(2.0) for forces, 1.0 for geometry spectral width\n\n";
    ...
    std::cout << "Original force RSS: " << original_force_rss << ", ZCS: " << original_force_zcs << "\n";
    ...
    std::cout << "Constrained force RSS: " << constrained_force_rss << ", ZCS: " << constrained_force_zcs << "\n";
    std::cout << "Energy conservation: " << (constrained_force_rss*constrained_force_rss + constrained_force_zcs*constrained_force_zcs) << "\n";
    std::cout << "Original energy: " << (original_force_rss*original_force_rss + original_force_zcs*original_force_zcs) << "\n\n";
...
TEST_F(ForceSpectralCondensationTest, VMECPPGeometryConstraintApplication) {
    ...
    std::cout << "Original geometry RSS: " << original_geom_rss << ", ZCS: " << original_geom_zcs << "\n";
    ...
    std::cout << "Constrained geometry RSS: " << constrained_geom_rss << ", ZCS: " << constrained_geom_zcs << "\n";
    std::cout << "Scaling difference vs jVMEC: " << (geom_scaling / (1.0/std::sqrt(2.0))) << "\n\n";
...
TEST_F(ForceSpectralCondensationTest, ConstraintTimingDifferences) {
    ...
    std::cout << "2. Force computation: constraint applied with scaling 1/sqrt(2)\n";
    std::cout << "3. Spectral width calculation: constraint applied with scaling 1.0\n";
    std::cout << "4. Each iteration: forces re-constrained during decomposition\n\n";
    
    std::cout << "VMEC++ TIMING:\n";
    std::cout << "1. Geometry initialization: constraint applied with scaling 0.5\n";
    ...
    std::cout << "D. Force vs geometry constraint application fundamentally different\n\n";
...
TEST_F(ForceSpectralCondensationTest, ConstraintForceMultiplierMissing) {
    ...
    std::cout << "Purpose: Dynamically calculated force scaling based on surface count\n";
    ...
    std::cout << "Impact: Fixed constraint strength regardless of surface count\n";
    std::cout << "Potential issue: Suboptimal constraint enforcement for high resolution\n\n";
...
TEST_F(ForceSpectralCondensationTest, BandPassFilteringMissing) {
    ...
    std::cout << "Potential issue: Unwanted m=0 and high-m constraint contributions\n\n";
    ...
    std::cout << "Additional modes in VMEC++: 2 extra modes contribute to constraint\n\n";
...
TEST_F(ForceSpectralCondensationTest, SymmetrizationDifferences) {
    ...
    std::cout << "Method: 0.5*(work[...] + work[...reversed]) for asymmetric case\n";
    std::cout << "Purpose: Enforce stellarator symmetry in constraint forces\n";
    std::cout << "Effect: Symmetric and anti-symmetric force components combined\n\n";
    ...
    std::cout << "Timing: Applied after force calculation, not during constraint\n";
    std::cout << "Potential difference: Order of operations may affect results\n\n";
    ...
    std::cout << "Anti-symmetric component: " << (0.5 * (force_normal - force_reversed)) << "\n\n";
