...
def additive_partition(
    ...
    """Take `axis_offset` surfaces off tid 0 and `lcfs_offset` off tid N-1,
    redistribute the surplus evenly across the interior threads.

    Mirrors the user's hypothesis that boundary overhead is additive
    (constant per-call) rather than multiplicative.
    """
    if n_threads == 1:
        return [n_surfaces]
    if n_threads == 2:
        ...
        if axis_offset >= lcfs_offset:
            return [n_surfaces // 2 - axis_offset // 2,
                    n_surfaces - (n_surfaces // 2 - axis_offset // 2)]
        return [n_surfaces - (n_surfaces // 2 - lcfs_offset // 2),
...
def weighted_partition(n_surfaces: int, weights: list[float]) -> list[int]:
    ...
    Each thread's *fair share* of the n_surfaces is proportional to
    1 / w[t]. Take the floor of every fair share (giving each thread at
    least its base allocation), then distribute the remaining surfaces to
    whichever threads have the largest fractional residual. This yields
    the same makespan as the binary-search-on-M optimum but never
    starves a thread (every thread gets at least floor(fair share)
    surfaces).

    Hamilton's method ties for makespan-optimality with binary-search-on-M
    while always preserving "every thread roughly its fair share", which
...
def run(n_surfaces: int, n_threads: int, w_axis: float, w_lcfs: float):
    ...
    return {
        ...
        "safe": Result(safe_partition, [c * w for c, w in zip(safe_partition, weights)], safe_ms),
...
def main():
    ...
    parser.add_argument("--w-axis", type=float, default=1.25, help="weight for the axis-owner thread (tid 0)")
    parser.add_argument("--w-lcfs", type=float, default=1.15, help="weight for the LCFS-owner thread (tid N-1)")
    parser.add_argument("--cases", type=str, default="99,199,299,499", help="comma-separated NS values")
    parser.add_argument("--threads", type=str, default="1,2,4,8,10,16,20,30,40", help="comma-separated thread counts")
    ...
    parser.add_argument("--axis-off", type=int, default=2,
                        help="Surfaces shifted off the axis owner (additive scheme).")
    parser.add_argument("--lcfs-off", type=int, default=1,
                        help="Surfaces shifted off the LCFS owner.")
    parser.add_argument("--b", type=float, default=18.5,
                        help="Per-surface cost (us) for additive model.")
    parser.add_argument("--a", type=float, default=392.0,
                        help="Per-call constant overhead (us) for additive model.")
    parser.add_argument("--delta-axis", type=float, default=125.0,
                        help="Boundary additive overhead at axis (us).")
    parser.add_argument("--delta-lcfs", type=float, default=78.0,
    ...
    if args.additive:
        ...
        for ns in [int(x) for x in args.cases.split(',')]:
            ...
            for t in [int(x) for x in args.threads.split(',')]:
                ...
                eq_ms = additive_makespan(eq, None, args.b, args.a,
                                          args.delta_axis, args.delta_lcfs)
                add_ms = additive_makespan(add, None, args.b, args.a,
                                           args.delta_axis, args.delta_lcfs)
                gain = 100.0 * (eq_ms - add_ms) / eq_ms
                marker = "  <- regress" if gain < 0 else ""
                print(f"{ns:>4} {t:>3} {eq_ms:>16.0f} {add_ms:>17.0f} "
